/****************************************************************************
** Meta object code from reading C++ file 'adjustTab.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.8.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../Widgets/adjustTab.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'adjustTab.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.8.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_CMyAdjustTab_t {
    QByteArrayData data[11];
    char stringdata0[146];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_CMyAdjustTab_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_CMyAdjustTab_t qt_meta_stringdata_CMyAdjustTab = {
    {
QT_MOC_LITERAL(0, 0, 12), // "CMyAdjustTab"
QT_MOC_LITERAL(1, 13, 15), // "ShowInfo_Signal"
QT_MOC_LITERAL(2, 29, 0), // ""
QT_MOC_LITERAL(3, 30, 18), // "HandleFrame_Signal"
QT_MOC_LITERAL(4, 49, 12), // "chSourceAddr"
QT_MOC_LITERAL(5, 62, 12), // "chSourcePort"
QT_MOC_LITERAL(6, 75, 13), // "ShowInfo_Slot"
QT_MOC_LITERAL(7, 89, 16), // "HandleFrame_Slot"
QT_MOC_LITERAL(8, 106, 13), // "AdjustMapFunc"
QT_MOC_LITERAL(9, 120, 13), // "ClearInfoFunc"
QT_MOC_LITERAL(10, 134, 11) // "PswLockFunc"

    },
    "CMyAdjustTab\0ShowInfo_Signal\0\0"
    "HandleFrame_Signal\0chSourceAddr\0"
    "chSourcePort\0ShowInfo_Slot\0HandleFrame_Slot\0"
    "AdjustMapFunc\0ClearInfoFunc\0PswLockFunc"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_CMyAdjustTab[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       7,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    2,   49,    2, 0x06 /* Public */,
       3,    3,   54,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       6,    2,   61,    2, 0x08 /* Private */,
       7,    3,   66,    2, 0x08 /* Private */,
       8,    1,   73,    2, 0x08 /* Private */,
       9,    0,   76,    2, 0x08 /* Private */,
      10,    0,   77,    2, 0x08 /* Private */,

 // signals: parameters
    QMetaType::Void, QMetaType::QString, QMetaType::QColor,    2,    2,
    QMetaType::Void, QMetaType::QString, QMetaType::UChar, QMetaType::UChar,    2,    4,    5,

 // slots: parameters
    QMetaType::Void, QMetaType::QString, QMetaType::QColor,    2,    2,
    QMetaType::Void, QMetaType::QString, QMetaType::UChar, QMetaType::UChar,    2,    4,    5,
    QMetaType::Void, QMetaType::Int,    2,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void CMyAdjustTab::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        CMyAdjustTab *_t = static_cast<CMyAdjustTab *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->ShowInfo_Signal((*reinterpret_cast< const QString(*)>(_a[1])),(*reinterpret_cast< QColor(*)>(_a[2]))); break;
        case 1: _t->HandleFrame_Signal((*reinterpret_cast< const QString(*)>(_a[1])),(*reinterpret_cast< unsigned char(*)>(_a[2])),(*reinterpret_cast< unsigned char(*)>(_a[3]))); break;
        case 2: _t->ShowInfo_Slot((*reinterpret_cast< const QString(*)>(_a[1])),(*reinterpret_cast< QColor(*)>(_a[2]))); break;
        case 3: _t->HandleFrame_Slot((*reinterpret_cast< const QString(*)>(_a[1])),(*reinterpret_cast< unsigned char(*)>(_a[2])),(*reinterpret_cast< unsigned char(*)>(_a[3]))); break;
        case 4: _t->AdjustMapFunc((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 5: _t->ClearInfoFunc(); break;
        case 6: _t->PswLockFunc(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (CMyAdjustTab::*_t)(const QString & , QColor );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&CMyAdjustTab::ShowInfo_Signal)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (CMyAdjustTab::*_t)(const QString & , unsigned char , unsigned char );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&CMyAdjustTab::HandleFrame_Signal)) {
                *result = 1;
                return;
            }
        }
    }
}

const QMetaObject CMyAdjustTab::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_CMyAdjustTab.data,
      qt_meta_data_CMyAdjustTab,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *CMyAdjustTab::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *CMyAdjustTab::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_CMyAdjustTab.stringdata0))
        return static_cast<void*>(const_cast< CMyAdjustTab*>(this));
    return QWidget::qt_metacast(_clname);
}

int CMyAdjustTab::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 7)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 7;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 7)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 7;
    }
    return _id;
}

// SIGNAL 0
void CMyAdjustTab::ShowInfo_Signal(const QString & _t1, QColor _t2)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void CMyAdjustTab::HandleFrame_Signal(const QString & _t1, unsigned char _t2, unsigned char _t3)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)), const_cast<void*>(reinterpret_cast<const void*>(&_t3)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
